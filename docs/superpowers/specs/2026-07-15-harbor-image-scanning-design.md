...
Harbor가 Artifact Push 이벤트를 `POST /api/harbor/webhook`으로 전송한다. JASCA는 서명 헤더를 HMAC SHA-256으로 검증하고, 이벤트 형식과 허용 프로젝트를 검증한다. 유효한 이미지 Digest는 수동 검사와 같은 공통 Trivy 이미지 검사 경로로 전달한다. 같은 Registry, Repository, Digest가 이미 실행 중이거나 최근 완료된 경우에는 새 작업을 만들지 않는다.
...
- 허용되지 않은 프로젝트, 유효하지 않은 HMAC, 지원하지 않는 이벤트는 `401` 또는 `403`으로 거절한다.
...
- 운영 Webhook은 JASCA의 인증된 외부 URL만 사용하고, Webhook Secret을 반드시 설정한다.
