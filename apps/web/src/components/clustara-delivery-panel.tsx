...
export function ClustaraDeliveryPanel({ scan }: { scan: Scan }) {
    ...
    const [imageDigest, setImageDigest] = useState(scan.imageDigest || '');
    ...
    useEffect(() => {
        ...
        setScanner(optionsQuery.data.scanner);
    ...
    const validDigest = DIGEST_PATTERN.test(imageDigest.trim());
    const canSubmit = Boolean(optionsQuery.data?.enabled && canSend && clusterId.trim() && validDigest);

    const queue = async (type: 'TRIVY' | 'SBOM') => {
        setMessage(null);
        try {
            await queueMutation.mutateAsync({ type, clusterId: clusterId.trim(), imageDigest: imageDigest.trim(), scanner: scanner.trim(), generator: generator.trim() });
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-6 dark:border-slate-700 dark:bg-slate-800">
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p className="mt-1 text-sm text-slate-500">원본 Trivy JSON과 보존된 Syft SBOM을 사내 Clustara에 등록합니다.</p>
                </div>
            ...
            <div className="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <Field label="image_digest" value={imageDigest} onChange={setImageDigest} placeholder={`sha256:${'0'.repeat(64)}`} error={imageDigest && !validDigest ? 'sha256: 뒤에 64자리 16진수를 입력하세요.' : undefined} />
            ...
            <div className="mt-5 flex flex-wrap gap-3">
                <button onClick={() => queue('TRIVY')} disabled={!canSubmit || queueMutation.isPending} className="inline-flex items-center gap-2 rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-40">
                    {queueMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Trivy JSON 전송
                </button>
