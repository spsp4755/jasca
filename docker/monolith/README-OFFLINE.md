...
1. `관리자 > Trivy DB`의 `OWASP ZAP 웹 스캔 설정`에서 전역 허용 대상 패턴을 먼저 저장합니다.
2. `ZAP 대상 프로필`을 추가하고 프로필 이름, 허용/차단 URL 패턴, 최대 실행 시간을 입력합니다.
3. `ZAP 연결 테스트`로 서버 버전 조회가 성공하는지 확인합니다. 이 동작은 스캔을 실행하지 않습니다.
4. 사용자는 ZAP 스캔 화면에서 활성 대상 프로필을 선택해야 합니다. 전역 정책과 프로필 정책을 모두 통과한 URL만 검사됩니다.

기존 설치본에 전역 허용 목록만 있으면 `기본 대상 프로필`이 자동으로 표시됩니다. 관리자 화면에서 내용을 검토하고 저장하면 명시적인 프로필로 관리할 수 있습니다. Cookie 또는 Authorization 값은 실행 중에만 ZAP 요청 헤더로 사용하며 저장하지 않습니다.
