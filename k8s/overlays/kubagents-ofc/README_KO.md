...
```text
harbor.kubagents-ofc.koreacb.com/jasca/jasca-offline:v0.3.13
```
...
폐쇄망 반입 파일 `jasca-offline-v0.3.13-amd64.tar.gz`를 사용합니다. Podman이 압축 파일을 직접 읽지 못하는 환경이면 `gzip -dc`로 표준 입력을 사용합니다.

```bash
gzip -dc jasca-offline-v0.3.13-amd64.tar.gz | podman load
podman images | grep jasca-offline

podman login harbor.kubagents-ofc.koreacb.com
podman tag jasca-offline:v0.3.13-amd64 \
  harbor.kubagents-ofc.koreacb.com/jasca/jasca-offline:v0.3.13
podman push harbor.kubagents-ofc.koreacb.com/jasca/jasca-offline:v0.3.13
```
